defmodule AppWeb.Components.UI.Timeline do
  ...
  def timeline(assigns) do
    ~H"""
      <ol class="relative border-s border-gray-900 dark:border-gray-100">
      <%= for entry <- @entries do %>
        <li class="mb-10 ms-4">
          <div class="absolute w-3 h-3 bg-gray-900 rounded-full mt-1.5 -start-1.5 border border-gray-900 dark:border-gray-100 dark:bg-gray-100"></div>
          <time class="mb-1 text-sm font-normal leading-none text-gray-900 dark:text-white"><%= entry.date %></time>
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><%= entry.title %></h3>
          <p class="mb-4 text-base font-normal text-gray-800 dark:text-gray-100"><%= entry.description %></p>
