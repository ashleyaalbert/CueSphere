    ...
    <div id={@id} data-accordion={if @multiple, do: "collapse", else: "single"}>
      <%= for {section, index} <- Enum.with_index(@sections) do %>
        <h2 id={"#{@id}-heading-#{index}"}>
          <button
            type="button"
            class="flex items-center justify-between w-full p-4 font-medium rtl:text-right text-gray-900 border border-b-0 border-gray-200 rounded-t-xl focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 dark:border-gray-700 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 bg-gray-50 dark:bg-gray-700 gap-3"
